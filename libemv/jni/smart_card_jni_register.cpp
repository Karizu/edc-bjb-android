...
{
	int nCount = 0;
	JNINativeMethod* pMethods = smart_card_get_methods(&nCount);

	return register_native_methods(env,	smart_card_get_class_name(),	pMethods, nCount);
}
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved)
{
	...
	{
		hal_sys_info("printer JNI_OnLoad(), failed in GetEnv()");
		return -1;
	}
	assert(env != NULL);
