...
typedef struct contactless_card_hal_interface {
    ...
    contactless_card_transmit_level3 transmit_level3;
...
static void contactless_card_callback(void *pUserData, int nEvent,
    ...
    if (g_jvm->GetEnv((void **) &env, JNI_VERSION_1_6) != JNI_OK) {
        ...
        hal_sys_info("Callback is running in java thread!!!");
    }
...
jint JNICALL native_contactless_card_open(JNIEnv *env, jclass obj) {
    ...
    if (g_pContactlessCard == NULL) {
        pSoHandle = dlopen("/system/lib/libwizarposDriver.so", RTLD_LAZY);
        if (!pSoHandle) {
            hal_sys_info("%s\n", dlerror());
            return ERR_NORMAL;
        }
        ...
        if (NULL == (g_pContactlessCard->open = (contactless_card_open) dlsym(pSoHandle,
                                                                              methodName = "contactless_card_open"))
            || NULL == (g_pContactlessCard->close = (contactless_card_close) dlsym(pSoHandle,
                                                                                   methodName = "contactless_card_close"))
            || NULL ==
               (g_pContactlessCard->search_target_begin = (contactless_card_search_target_begin) dlsym(
                       pSoHandle, methodName = "contactless_card_search_target_begin"))
            || NULL ==
               (g_pContactlessCard->search_target_end = (contactless_card_search_target_end) dlsym(
                       pSoHandle, methodName = "contactless_card_search_target_end"))
            || NULL == (g_pContactlessCard->attach_target = (contactless_card_attach_target) dlsym(
                pSoHandle, methodName = "contactless_card_attach_target"))
            || NULL == (g_pContactlessCard->transmit = (contactless_card_transmit) dlsym(pSoHandle,
                                                                                         methodName = "contactless_card_transmit"))

            || NULL == (g_pContactlessCard->detach_target = (contactless_card_detach_target) dlsym(
                pSoHandle, methodName = "contactless_card_detach_target"))
            || NULL ==
               (g_pContactlessCard->send_control_command = (contactless_card_send_control_command) dlsym(
                       pSoHandle, methodName = "contactless_card_send_control_command"))
            || NULL == (g_pContactlessCard->mc_verify_pin = (contactless_card_mc_verify_pin) dlsym(
                pSoHandle, methodName = "contactless_card_mc_verify_pin"))
            || NULL == (g_pContactlessCard->mc_read = (contactless_card_mc_read) dlsym(pSoHandle,
                                                                                       methodName = "contactless_card_mc_read"))
            || NULL == (g_pContactlessCard->mc_write = (contactless_card_mc_write) dlsym(pSoHandle,
                                                                                         methodName = "contactless_card_mc_write"))) {
            hal_sys_info("can't find %s", methodName);
            nErrorCode = ERR_NO_IMPLEMENT;
            goto card_open_clean;
        }
        if (NULL == (g_pContactlessCard->query_info = (contactless_card_query_info) dlsym(pSoHandle,
                                                                                          methodName = "contactless_card_query_info"))
            || NULL == (g_pContactlessCard->mc_read_value = (contactless_card_mc_read_value) dlsym(
                pSoHandle, methodName = "contactless_card_mc_read_value"))
            || NULL ==
               (g_pContactlessCard->mc_write_value = (contactless_card_mc_write_value) dlsym(
                pSoHandle, methodName = "contactless_card_mc_write_value"))
			|| NULL == (g_pContactlessCard->transmit_level3 = (contactless_card_transmit_level3) dlsym(
				pSoHandle,methodName = "contactless_card_transmit_level3"))
            || NULL ==
               (g_pContactlessCard->mc_increment = (contactless_card_mc_increment) dlsym(pSoHandle,
                                                                                         methodName = "contactless_card_mc_increment"))
            || NULL ==
               (g_pContactlessCard->mc_decrement = (contactless_card_mc_decrement) dlsym(pSoHandle,
                                                                                         methodName = "contactless_card_mc_decrement"))
            || NULL ==
               (g_pContactlessCard->mc_transfer = (contactless_card_mc_transfer) dlsym(pSoHandle,
                                                                                       methodName = "contactless_card_mc_transfer"))
            || NULL ==
               (g_pContactlessCard->mc_restore = (contactless_card_mc_restore) dlsym(pSoHandle,
                                                                                     methodName = "contactless_card_mc_restore"))
            || NULL == (g_pContactlessCard->touch_card = (contactless_card_final_touch_card) dlsym(
                pSoHandle, methodName = "contactless_card_final_touch_card"))) {
            hal_sys_info("can't find %s", methodName);
        }
        ...
        pCardHandle = g_pContactlessCard->open(contactless_card_callback, g_pContactlessCard,
                                               &nErrorCode);
        hal_sys_info("g_pContactlessCard->open ... result is %d", pCardHandle);
        if (pCardHandle == NULL || nErrorCode < 0) {
            goto card_open_clean;
        }
        g_pContactlessCard->pCardHandle = (long) pCardHandle;
    }
...
jint JNICALL native_contactless_card_search_target_begin(JNIEnv *env, jclass obj,
                                                         jint nCardMode, jint nFlagSearchAll,
                                                         jint nTimeout_MS) {
    ...
    nResult = g_pContactlessCard->search_target_begin(g_pContactlessCard->pCardHandle, nCardMode,
                                                      nFlagSearchAll, nTimeout_MS);
...
jint JNICALL native_contactless_card_transmit(JNIEnv *env, jclass obj,
                                              jbyteArray arryAPDU, jint nAPDULength,
                                              jbyteArray arryResponse) {
    ...
    nResult = g_pContactlessCard->transmit(g_pContactlessCard->pCardHandle,
                                           (unsigned char *) pData, nAPDULength,
                                           (unsigned char *) pResponse, &nResponseBufferLength);
...
jint JNICALL native_contactless_card_transmit_level3(JNIEnv *env, jclass obj,
                                              jbyteArray arryAPDU, jint nAPDULength,
                                              jbyteArray arryResponse,jint nOptionValue) {
    hal_sys_info("+ native_contactless_card_transmit_level3()");
    int nResult = ERR_NORMAL;
    if (g_pContactlessCard == NULL)
        return ERR_NOT_OPENED;
    if (g_pContactlessCard->transmit_level3 == NULL)
        return ERR_NO_IMPLEMENT;
    if (arryAPDU == NULL || arryResponse == NULL)
        return ERR_INVALID_ARGUMENT;
    ...
    unsigned int nResponseBufferLength = (unsigned int) (env->GetArrayLength(arryResponse));
    nResult = g_pContactlessCard->transmit_level3(g_pContactlessCard->pCardHandle,
                                           (unsigned char *) pData, nAPDULength,
                                           (unsigned char *) pResponse, &nResponseBufferLength,nOptionValue);
    env->ReleaseByteArrayElements(arryAPDU, pData, 0);
    env->ReleaseByteArrayElements(arryResponse, pResponse, 0);
    nResult = nResult >= 0 ? nResponseBufferLength : nResult;
    hal_sys_info("- native_contactless_card_transmit_level3(), result = %d", nResult);
    return nResult;
}

jint JNICALL native_contactless_card_send_control_command(JNIEnv *env, jclass obj,
                                                          jint nCmdID, jbyteArray arryCmdData,
                                                          jint nDataLength) {
...
jint JNICALL native_contactless_card_mc_verify_pin(JNIEnv *env, jclass obj,
                                                   jint nSectorIndex, jint nPinType,
                                                   jbyteArray strPin, jint nPinLength) {
    ...
    nResult = g_pContactlessCard->mc_verify_pin(g_pContactlessCard->pCardHandle,
                                                (unsigned int) nSectorIndex,
                                                (unsigned int) nPinType,
...
jint JNICALL native_contactless_card_mc_write(JNIEnv *env, jclass obj,
                                              jint nSectorIndex, jint nBlockIndex, jbyteArray pData,
                                              jint nDataLength) {
...
jint JNICALL native_contactless_card_query_info(JNIEnv *env, jclass obj,
    ...
    nResult = g_pContactlessCard->query_info(g_pContactlessCard->pCardHandle, iHasMoreCards,
                                             iCardType);
...
jboolean native_contactless_card_is_opened(JNIEnv *env, jclass obj) {
    jboolean isOpened = JNI_FALSE;
    hal_sys_info("native_contactless_card__is_opened() is called\n");
    if (g_pContactlessCard != NULL) {
        isOpened = JNI_TRUE;
    }
    hal_sys_info("native_contactless_card__is_opened() end result = %d", isOpened);
    return isOpened;
}

jint JNICALL native_contactless_card_get_handle(JNIEnv *env, jclass obj) {
    int nResult = ERR_NOT_OPENED;
    hal_sys_info("native_contactless_card_get_handle() is called\n");
    if (g_pContactlessCard != NULL) {
        nResult = g_pContactlessCard->pCardHandle;
    }
    hal_sys_info("native_contactless_card_get_handle() end result = %d", nResult);
    return nResult;
}
