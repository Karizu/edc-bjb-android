...
{
	int nCount = 0;
	JNINativeMethod* pMethods = osm_get_methods(&nCount);
	jint nResult = register_native_methods(env,	osm_get_class_name_cloudpos(),	pMethods, nCount);
	if(nResult == JNI_FALSE){
		env->ExceptionClear();
		nResult = register_native_methods(env,	get_class_name_internal(),	pMethods, nCount);
		if(nResult == JNI_FALSE){
			env->ExceptionClear();
			nResult = register_native_methods(env,	osm_get_class_name_wizarpos(),	pMethods, nCount);
		}
//
	}
	return nResult;
