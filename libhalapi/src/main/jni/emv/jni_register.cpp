...
{
	int nCount = 0;
	JNINativeMethod* pMethods = emv_kernal_get_methods(&nCount);

	jint nResult = register_native_methods(env,	get_class_name(), pMethods, nCount);
	if(nResult == JNI_FALSE){
		env->ExceptionClear();
		pMethods = emv_kernal_get_methods(&nCount);
		nResult = register_native_methods(env,	get_internal_class_name(),	pMethods, nCount);
	}
	return nResult;
}
