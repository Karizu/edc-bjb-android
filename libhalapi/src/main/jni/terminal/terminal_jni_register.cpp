...
{
	int nCount = 0;
	JNINativeMethod* pMethods = get_methods(&nCount);

	int nResult = register_native_methods(env,	get_class_name(),	pMethods, nCount);
	//	+update by pengli
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved)
{
	...
	{
		hal_sys_info("Terminal Status JNI_OnLoad(), failed in GetEnv()");
		return -1;
	}
	assert(env != NULL);
