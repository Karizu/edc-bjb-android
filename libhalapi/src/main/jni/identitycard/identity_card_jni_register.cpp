...
{
	int nCount = 0;
	JNINativeMethod* pMethods = identity_card_get_methods(&nCount);

	jint nResult = register_native_methods(env,	identity_card_get_class_name(),	pMethods, nCount);
	//	+update by pengli
	if(nResult == JNI_FALSE){
			env->ExceptionClear();
			pMethods = identity_card_get_methods_internal(&nCount);
