...
int native_printer_query_voltage(JNIEnv *env, jclass obj,
    ...
    g_pPrinterInstance->query_voltage = (printer_query_voltage) dlsym(pHandle,
                                                                      "printer_query_voltage");
...
int native_printer_open(JNIEnv *env, jclass obj) {
    ...
    if (g_pPrinterInstance == NULL) {
        ...
        char *methodName;
        if (NULL ==
            (g_pPrinterInstance->open = (printer_open) dlsym(pHandle, methodName = "printer_open"))
            || NULL == (g_pPrinterInstance->close = (printer_close) dlsym(pHandle,
                                                                          methodName = "printer_close"))
            || NULL == (g_pPrinterInstance->begin = (printer_begin) dlsym(pHandle,
                                                                          methodName = "printer_begin"))
            || NULL ==
               (g_pPrinterInstance->end = (printer_end) dlsym(pHandle, methodName = "printer_end"))
            || NULL == (g_pPrinterInstance->write = (printer_write) dlsym(pHandle,
        ...
        if (NULL == (g_pPrinterInstance->query = (printer_query_status) dlsym(pHandle,
                                                                              methodName = "printer_query_status"))) {
            hal_sys_error("can't find %s", methodName);
        }

        if (NULL == (g_pPrinterInstance->read = (printer_read) dlsym(pHandle,
                                                                     methodName = "printer_read"))) {
            hal_sys_error("can't find %s", methodName);
        }
...
int
native_printer_read(JNIEnv *env, jclass obj, jbyteArray arryData, jint nDataLength, jint nTimeout) {
