...
{
	int nCount = 0;
	JNINativeMethod* pMethods = ac5_get_methods(&nCount);

	int nResult = register_native_methods(env,	ac5_get_class_name(),	pMethods, nCount);
	//	+update by pengli
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved)
{
	...
	{
		hal_sys_info("ac5 JNI_OnLoad(), failed in GetEnv()");
		return -1;
	}
	assert(env != NULL);
